...
<template>
  <div class="progress-circle-base relative size-40 text-2xl font-semibold" :class="[props.class]">
    ...
    <span v-if="showPercentage" :data-current-value="currentPercent" class="animate-in fade-in absolute inset-0 m-auto size-fit delay-0 duration-1000 ease-linear">
      {{ currentPercent }}
    </span>
  </div>
</template>
